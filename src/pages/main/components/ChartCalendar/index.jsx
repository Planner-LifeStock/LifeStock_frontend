...
const DayContainer = styled.div`
  ...
  border: ${({ $isSelected }) => ($isSelected ? `solid 3px red` : `solid 1px black`)};
...
function ChartCalendar() {
  ...
  const renderCalendar = () => {
    ...
    weekDays.forEach((weekDay, index) => {
      days.push(<DayOfWeekContainer key={`weekday-${index}`}>{weekDay}</DayOfWeekContainer>);
    });
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      days.push(
        ...
        >
          ...
          {displayChangeRate && <ChangeRateText color={changeRateColor}>{displayChangeRate}%</ChangeRateText>}
        </DayContainer>
      );
    }
